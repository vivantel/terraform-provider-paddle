...
func TestSameChargeItems_OrderIndependentExactMatch(t *testing.T) {
	...
	got := []client.TransactionItem{
		{Price: client.TransactionItemPrice{ID: "pri_2"}, Quantity: 3},
		{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 1},
	}
...
func TestSameChargeItems_DifferentQuantityDoesNotMatch(t *testing.T) {
	...
	got := []client.TransactionItem{{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 2}}
	if sameChargeItems(want, got) {
...
func TestSameChargeItems_DifferentLengthDoesNotMatch(t *testing.T) {
	...
	got := []client.TransactionItem{{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 1}}
	if sameChargeItems(want, got) {
...
func TestSameChargeItems_DuplicatePriceIDsMatchOneToOne(t *testing.T) {
	...
	gotOneOnly := []client.TransactionItem{{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 1}}
	if sameChargeItems(want, gotOneOnly) {
		t.Error("sameChargeItems = true, want false — only one of the two wanted items is present")
	}
	gotBoth := []client.TransactionItem{
		{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 1},
		{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 2},
	}
...
func TestFindMatchingCharge_MatchesOnExactItemSet(t *testing.T) {
	existing := []client.Transaction{
		{ID: "txn_1", Items: []client.TransactionItem{{Price: client.TransactionItemPrice{ID: "pri_1"}, Quantity: 1}}},
		{ID: "txn_2", Items: []client.TransactionItem{{Price: client.TransactionItemPrice{ID: "pri_2"}, Quantity: 5}}},
	}
